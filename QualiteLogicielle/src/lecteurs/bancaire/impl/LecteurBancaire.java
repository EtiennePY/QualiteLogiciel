...
public class LecteurBancaire implements ILecteurBancaire {
  ...
  public boolean demandeInsertionCarte(final boolean ticketOk) {
    if (ticketOk) {
      ...
      LOG.warning("Appel à l'écran du lecteur de carte bancaire"
          + " sans que le ticket ne soit valide au prealable.");
    }
    return ticketOk;
  }
  ...
  public boolean restitutionCarteBancaire(final boolean retour) {
    if (retour) {
      ...
      LOG.info("Un message s'affiche : \"OK\" sur l'écran du"
          + " lecteur de carte bancaire et la carte sort");
    } else {
      LOG.info("Un message s'affiche : \"Carte erronée\""
          + " sur l'écran du lecteur de carte bancaire et la carte sort");
    }
    return retour;
  }
   ...
   * 
   */
  public boolean realiseTransaction(final ISystemeInformatique sys, 
       final ILecteurTicket lecteurTicket, final IBarriereSortie barriere)
           throws TicketErreur, BarriereErreur, CarteBancaireErreur {
    if (lecteurTicket.getTicketClient().isWith()) {
      ...
      if (this.getCarteBancaire().isWith()) {
        final boolean transaction = 
            this.contacterBanque((ICarteBancaire) this.getCarteBancaire(), prix);
