...
public class MockLecteurCarteAbonnement implements ILecteurCarteAbonnement {
  ...
  /**
   * Instantiates a new mock lecteur carte abonnement.
   */
  ...
  public boolean restitutionCarte(final boolean retour) { 
    if (retour) {
      ...
      LOG.warning("Un message s'affiche : \"Carte erronée\""
          + " sur l'écran du lecteur et la carte sort");
    }
    return retour;
  }
  ...
  public boolean demandeInsertionCarte(final boolean detecte) {
    if (detecte) {
      LOG.info("L'écran du lecteur "
          + "de carte d'abonnement affiche \"Insérez votre carte d'abonnement\"");
    } else {
   ...
   * 
   */
  public boolean checkAbonnement(final ISystemeInformatique sys,
